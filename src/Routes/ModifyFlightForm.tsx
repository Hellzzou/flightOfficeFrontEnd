...
import {
	AddPilot,
	formValidity,
	fullfillFlightFormWithValidatedFlight,
...
export const ModifyFlightForm = ({
	...
	const handleReturnClick = () => history.push("/modifyFlightTable")

	const handleConfirmModalOpen = () => {
		setConfirmArray("Etes-vous sûr de vouloir modifier ce vol ?")
	...
	async function validateflight() {
		...
		if (deleted) {
			const saved = await saveValidatedFlight(
				hooks,
				durations,
				pilots,
				additionals,
			...
			if (saved) {
				...
				history.push("/modifyFlightTable")
			}
		}
	}
	...
	useAsyncEffect(() => {
		;(async () => {
			...
			if (typeof match.params.id !== "undefined")
				fullfillFlightFormWithValidatedFlight(match.params.id, setters)
		})()
